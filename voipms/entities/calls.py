...
class Calls(BaseApi):
    ...
    def _cdr(self, method, date_from, date_to, timezone,
        ...
        if "account" in kwargs:
            if not isinstance(kwargs["account"], int):
                raise ValueError("Filter CDR by Account needs to be an int (Values from calls.get_call_accounts)")
    ...
    def get_rates(self, package, query):
        """
    ...
    def get_termination_rates(self, route, query):
        """
        Retrieves the Rates for a specific Route (Premium, Value) and a Search term

        :param package: [Required] Route Code (Values from accounts.get_routes)(Example: '2')
        :type package: :py:class:`int`
        :param query: [Required] Query for searching rates (Example: 'Canada')
        :type query: :py:class:`str`
        :returns: :py:class:`dict`
        ...
        if not isinstance(route, int):
            raise ValueError("Route Code needs to be an int (Values from accounts.get_routes)(Example: '2')")
    ...
    def get_reseller_cdr(self, date_from, date_to, client, timezone,
                         answered=False, noanswer=False, busy=False,
                         failed=False, **kwargs):
        """
        Retrieves the Call Detail Records of all your calls
